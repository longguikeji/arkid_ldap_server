...
export default function (domain) {
  ...
  return function (req, res, next) {
    ...
    logger.debug(req.connection.ldap.bindDN.toString())
    logger.debug(cache.getKey(req.connection.ldap.bindDN.toString())["token"]);
    logger.debug(parsedUnit[1], parsedUnit[2]);

    const token = cache.getKey(req.connection.ldap.bindDN.toString()).token;
    var req_params = {};
